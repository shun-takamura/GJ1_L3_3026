...
/// outSpawns に積む(Pistol.cpp と同じ単発パターン)。slowMultiplier/slowDuration を追加で
/// 埋めている点だけが違う ── ArcingProjectile::TryHitCharacter がこれをそのまま
/// Character::AttackHitbox へコピーし、命中した相手の Character::ReceiveHit が
/// ApplySlow() を呼ぶ(ProjectileSpawnRequest.h の設計コメント参照)。
/// </summary>
bool IceGun::TryRangedAttack(float dt, bool triggered, bool held, const Vector3& ownerPos,
	...
	spawn.slowMultiplier = kSlowMultiplier;
	spawn.slowDuration = kSlowDuration;
	outSpawns.push_back(spawn);
	return true;
}

void IceGun::DrawImGuiTuning() {
#ifdef USE_IMGUI
	...
	ImGui::Separator();
	ImGui::DragFloat("Slow Multiplier", &kSlowMultiplier, 0.02f, 0.05f, 0.95f);
	ImGui::DragFloat("Slow Duration (s)", &kSlowDuration, 0.1f, 0.1f, 10.0f);
#endif
}
