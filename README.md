...
**1. リポジトリのセットアップ（1回だけ・全員必須）**

`Project/tools/mergiraf/setup_mergiraf.bat` をエクスプローラーでダブルクリック。

2つのことをする。

- **mergiraf の登録** — `git merge` は行単位でしか見ないので、**二人が同じクラスに別々の関数を足しただけ**でもコンフリクトになる。mergiraf は C++ を構文木として解析してこれを自動で解決する
- **`imgui.ini` をローカル管理に切り替え** — エディタのウィンドウ配置。リポジトリには整った初期配置が入っているが、各自が動かした結果を git に載せると全員で衝突する。`skip-worktree` を付けて**ローカルの変更を git に見せない**ようにする

どちらも設定先（`.git/config` とインデックス）がバージョン管理されないため、**各自が1回ずつ実行する必要がある**。

> **`imgui.ini` は自由に動かしてよい。** git には反映されないので、他の人の配置を壊す心配はない。
> 逆に「全員の初期配置を更新したい」ときは、`git update-index --no-skip-worktree Project/imgui.ini`
> で一時的に戻してからコミットする。
